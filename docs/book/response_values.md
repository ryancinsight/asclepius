...
## Volume effect and response slope
...
architectures where gEUD approaches the minimum dose. `ResponseSlope<T>` is a
finite positive dose-response slope — for TCP/NTCP models, the normalized
slope `gamma50` at the 50% response point. Both reject their zero or
non-positive degenerate cases at construction:

```rust,ignore
let a_parallel = VolumeEffect::<f64>::new(1.0)?;   // parallel (mean-like)
let a_serial = VolumeEffect::<f64>::new(-16.0)?;   // serial (min-like)
let gamma50 = ResponseSlope::<f64>::new(4.0)?;
assert!(VolumeEffect::<f64>::new(0.0).is_err());
assert!(ResponseSlope::<f64>::new(-1.0).is_err());
```
